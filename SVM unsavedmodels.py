...
### Organising the data to create a bag of words model
...
data_set = data_set.assign(posts=data_set.posts.apply(post_cleaner)).dropna()
print(data_set)
#####
...
# print(nltk.classify.util.accuracy(sentiment_classifier, test)*100)
###################################
...
IntroExtro = SklearnClassifier(LinearSVC()).train(train)

print("Training Accuracy of IntroExtro",nltk.classify.util.accuracy(IntroExtro, train)*100)

test=[]
for i in range(16):
    test += features[i][split[i]:]
print("Testing Accuracy of IntroExtro",nltk.classify.util.accuracy(IntroExtro, test)*100)
#import pickle
# f = open('IntroExtro.pickle', 'wb')
...
IntuitionSensing = IntroExtro = SklearnClassifier(LinearSVC()).train(train)
#IntuitionSensing.train(features)
print("Training Accuracy of IntuitionSensing",nltk.classify.util.accuracy(IntuitionSensing, train)*100)
...
for i in range(16):
    test += features[i][split[i]:]
print("Testing Accuracy of IntuitionSensing",nltk.classify.util.accuracy(IntuitionSensing, test)*100)
# f = open('IntuitionSensing.pickle', 'wb')
...
ThinkingFeeling = IntroExtro = SklearnClassifier(LinearSVC()).train(train)
#ThinkingFeeling.train(features)
print("Training Accuracy of ThinkingFeeling",nltk.classify.util.accuracy(ThinkingFeeling, train)*100)

test=[]
for i in range(16):
    test += features[i][split[i]:]
print("Testing Accuracy of ThinkingFeeling",nltk.classify.util.accuracy(ThinkingFeeling, test)*100)
# f = open('ThinkingFeeling.pickle', 'wb')
...
JudgingPercieiving = IntroExtro = SklearnClassifier(LinearSVC()).train(train)
#JudgingPercieiving.train(features)

print("Training Accuracy of JudgingPercieving",nltk.classify.util.accuracy(JudgingPercieiving, train)*100)
test=[]
for i in range(16):
    test += features[i][split[i]:]
print("Testing Accuracy of JudgingPercieving",nltk.classify.util.accuracy(JudgingPercieiving, test)*100)
# f = open('JudgingPercieiving.pickle', 'wb')
...
temp = {'train' : [87.9089601208897,72.95446322427249,87.97002597886501,87.00125236759575], 'test' : [57.538385134296234,53.32480531430238,57.596324959649046,53.43707321110789]}
...
class App(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.title = 'Personality Prediction through Text'
...
"")
    ...
    def on_click(self):
        ...
        if trait=='INFJ':
            exp='Seek meaning and connection in ideas, relationships, and material possessions. Want to understand what motivates people and are insightful about others. Conscientious and committed to their firm values.\n Develop a clear vision about how best to serve the common good. Organized and \ndecisive in implementing their vision.'
